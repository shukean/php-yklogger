...
foreach ($classes as $key => $value) {
    if (stripos($value, 'ykloger') !== false || stripos($value, 'Bqlogger') !== false){
        continue;
    }
    unset($classes[$key]);
}
...
foreach(get_defined_functions()['internal'] as $k => $func){
    if (strpos($func, 'ykloger') !== false){
        echo $func, PHP_EOL;
    }
}

foreach(get_defined_constants() as $k => $consts){
    if (strpos($k, 'YKLOGER') !== false){
        echo 'define(\''.$k.'\', '.$consts.');', PHP_EOL;
    }
}
